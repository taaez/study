...
int main()
{
	...
	OAHT_Set(&HT, "CSCO", "Cisco System, Inc.");
	OAHT_Set(&HT, "GOOG", "Google Inc.");
	OAHT_Set(&HT, "YHOO", "Yahoo! Inc.");
	OAHT_Set(&HT, "NOVL", "Novell, Inc.");
	OAHT_Set(&HT, "NVOL", "Loop Check");
	OAHT_Set(&HT, "VOLN", "Loop Check 2"); // 충돌이 2번 발생할 때 루프 체크
	...
	printf("Key:%s, Value:%s\n", "NVOL", OAHT_Get(HT, "NVOL"));
	printf("Key:%s, Value:%s\n", "VOLN", OAHT_Get(HT, "VOLN"));
